...
(async () => {
    ...
    // Fungsi untuk memuat file remote (dapat digunakan ulang)
    async function loadFiles() {
        ...
        if (urls.length === 0 || referers.length === 0 || userAgents.length === 0) {
            console.error("Salah satu file kosong. Pastikan file yang diperlukan tidak kosong.");
            process.exit(1);
        }

        return { urls, referers, userAgents };
    }

    // Load files untuk pertama kali
    let { urls, referers, userAgents } = await loadFiles();
    ...
    while (iteration <= maxIterations) {
        ...
        // Update file secara otomatis pada setiap iterasi
        if (iteration % 100 === 0) {  // Contoh: perbarui file setiap 100 iterasi
            console.log("Memperbarui file URL, Referer, dan User-Agent...");
            const files = await loadFiles();
            urls = files.urls;
            referers = files.referers;
            userAgents = files.userAgents;
        }
